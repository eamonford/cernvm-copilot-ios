...
{
    CGRect touchesArea = CGRectMake(self.frame.size.width/2-self.size.width/2+self.xOffset, self.frame.size.height/2-self.size.height/2+self.yOffset, self.size.width, self.size.height);
    CGPoint touchLocation = [recognizer locationInView:self];
    
    if (CGRectContainsPoint(touchesArea, touchLocation)) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(hudWasTapped:)]) {
            [self.delegate hudWasTapped:self];
        }
    }
}
